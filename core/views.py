...
from core.models import UserCustom  # Asegúrate de importar UserCustom
import os
...
def create_task(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            task.project_id = form.cleaned_data['project'].id  # Asigna el proyecto seleccionado
            task.save()
...
def handle_uploaded_file(file, file_name, upload_to):
    with open(os.path.join(settings.MEDIA_ROOT, upload_to, file_name), 'wb+') as destination:
