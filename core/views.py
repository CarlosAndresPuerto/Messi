...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if password == 'psicologosenaadmin':
...
def projects(request):
    ...
    if user.is_authenticated and user.is_teacher:
        ...
        # Para estudiantes o usuarios no autenticados, mostrar todos los proyectos
...
def create_project(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if Project.objects.filter(name=project_name, teacher=teacher).exists():
                messages.error(request, f'Ya existe un proyecto con el nombre "{project_name}".')
...
def project_detail(request, project_id):
    ...
    # Verifica si el usuario es un profesor y es el creador del proyecto
    ...
    if user_is_teacher and project.teacher.user == request.user:
        ...
        # Accede al atributo directamente, ya que es un archivo individual
    ...
    # Si el usuario no es un profesor o no es el creador, permitir el acceso al estudiante
    # Aquí podrías agregar lógica específica para estudiantes, si es necesario
    tasks = Task.objects.filter(project_id=project_id)
    # Accede al atributo directamente, ya que es un archivo individual
    attachment = project.archivos_adjuntos
    return render(request, 'project_detail.html', {'project': project, 'tasks': tasks, 'attachment': attachment})
...
def delete_task_attachment(request, task_id):
    ...
    # Asegúrate de manejar la lógica para eliminar el archivo adjunto
...
def submit_task(request, task_id):
    ...
    # Deshabilitar el campo de calificación para el estudiante
